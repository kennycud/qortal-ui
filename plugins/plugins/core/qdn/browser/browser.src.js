	...
	firstUpdated() {
		...
		window.addEventListener('message', async (event) => {
			...
			switch (data.action) {
				...
				case actions.SIGN_FOREIGN_FEES: {

					const node = window.parent.reduxStore.getState().app.nodeConfig.knownNodes[window.parent.reduxStore.getState().app.nodeConfig.node]
					const nodeUrl = node.protocol + '://' + node.domain + ':' + node.port

					const signRequest = await showModalAndWait(
						actions.SIGN_FOREIGN_FEES,
						{
							text1: get("modals.mpchange88")
						}
					)
					if (signRequest.action === 'accept') {
						...
						let unsignedFeesUrl = `${nodeUrl}/crosschain/unsignedfees/${address}`
						...
						unsignedFees.forEach((unsignedFee) => {
							...
							let unsignedDataDecoded = Base58.decode(unsignedFee.data);
			...
			<div class="modal my-modal-class">
				<div class="modal-content">
					<div class="modal-body">
						...
						${type === actions.SIGN_FOREIGN_FEES ? `
